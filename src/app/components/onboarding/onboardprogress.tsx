...
const OnboardProgress = (props: OnboardProgressProps) => {
  const { currentStep, returnToBasic, returnToApp } = props;
  ...
  return (
    <section className={styles.container}>
      {Object.entries(progress).map(([k, v], index) => {
        return (
          <div
            style={{ backgroundColor: `#${v}`, color: `#${v}`, borderColor: `#${v}` }}
            className={styles.wrapper}
            key={index}
            onClick={() => {
              if (k === "Basic_Info" && currentStep !== 1) {
                returnToBasic();
              } else if (k === "Your_Posting" && currentStep !== 2) {
                returnToApp();
              } 
            }}
          >
            <p
              style={v === "D9D9D9" ? { color: "black" } : { color: "white" }}
